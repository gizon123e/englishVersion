...
function tryAgain(){
    ...
    if(inputPlayer == computer){
        ...
        konten.textContent = "The number is too low!\nRemaining chances: " + (kesempatan - 1);
        kesempatan --;
    }else if(inputPlayer > computer){
        var konten = document.querySelector('#cobaLagi h1');
        konten.textContent = "The number is too high!\nRemaining chances: " + (kesempatan - 1);
        kesempatan --;
    }

    if(kesempatan == 0){
        alert('Unfortunately, you have no more chances\nThe number is ' + computer);
        location.reload();
        return;
    }
}